...
def get_random_lines(x, y, N):
    ...
    for n in range(N):
        ...
        x1, x2 = np.random.uniform(x[0], x[-1], 2)
        y1, y2 = np.random.uniform(y[0], y[-1], 2)
        #get the vector
        vx = x2 - x1
        vy = y2 - y1
        #get the centre
        cx = (x1 + x2)/2.0
        cy = (y1 + y2)/2.0
        #extend from the centre of my line
        v = np.sqrt(vx**2 + vy**2)
        vx /= v
        vy /= v
        x1 = cx - l*vx
...
def analysis(lengths, bin_width, dc, name):
    """statistics"""
    if not lengths:
        raise RuntimeError("lenghts list is empty")
        #raw data
